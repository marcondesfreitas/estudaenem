                  ...
                  <div class='tudo'>
                    ...
                    <fieldset id='meuMenu'>
                        ...
                        <div class='formulario'>
                          <form action='../adicionar/add_sociologia.php' enctype='multipart/form-data' method='POST'>
...
if (isset($_SESSION['email'])) {
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    if (mysqli_num_rows($result) > 0) {
      while($row = mysqli_fetch_assoc($result)) {
        // Exibir a imagem
        $sql = mysqli_query($conn, "SELECT imagem FROM sociologia");
        if (!$sql) {
          die('Erro na consulta SQL: ' . mysqli_error($conn));
        }
        $imagem = $row["imagem"];

        echo "<form action='../editar_pg/edt_sociologia.php' method='post' style='display:inline;'>"; // Adicionar o formulário de edição
...
if (isset($_POST['apagar'])) {
  ...
  if(isset($_POST['id'])) {
    ...
    $sql = "DELETE FROM sociologia WHERE id = $id";
    $result = mysqli_query($conn, $sql);
    if ($result) {
    } else {
